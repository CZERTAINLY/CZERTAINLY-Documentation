...
Displays custom attributes that have been defined for the **Signing Profile** object in **Settings → Custom Attributes** in ILM. Custom attributes can be defined for different object types — only those scoped to Signing Profile appear here. Once defined, they appear on this tab and you can set values for them.
