...
A Signing Profile binds a number of other resources — its TSA signing certificate, Token Profile, RA Profile, delegated-signer and signature-formatter connectors, Time Quality Configuration, and TSP Profile (see the [entity-relationship diagram on the Signing Profile page](/docs/signing/signing-profile#relationships)). While any of these is still referenced, the platform **will not let it be deleted**, so a profile can never be left pointing at something that no longer exists.
