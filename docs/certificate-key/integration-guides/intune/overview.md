...
ILM supports SCEP, you can use Intune SCEP certificate profiles (a type of device profile in Intune) to deploy the certificates to your devices using the SCEP protocol and the ILM SCEP service. For more information about SCEP support in ILM, see the [SCEP overview](../../protocols/scep/overview.md).
...
This integration guide assumes at least basic knowledge about the Intune and does not provide guidance on how to install Intune. It focuses on the steps necessary to do to make the integration working with the ILM SCEP service. For more information about the Intune installation, refer to Intune documentation.
:::

In order to use ILM as a SCEP service for Intune, you need to follow these steps:
