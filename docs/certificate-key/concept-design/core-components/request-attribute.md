...
- **Certificate extension** — an X.509 extension identified by its OID from the [OID registry](../../settings/oid.md). The OID must be registered in the `Certificate Extension` category before an attribute can map to it — a definition referencing an unregistered OID is rejected when saved. The registry entry provides the default criticality and the value encoding used to turn the string value into the extension value.
