...
- **RDN (subject)** — a component of the certificate subject name. The RDN is identified by its code (for example `CN`) or its dotted-decimal OID, resolved through the [OID registry](../../settings/oid.md). Subject components render in the order their definitions appear in the set, and the same RDN type can appear more than once (multivalued subjects).
