...
This page covers immediate issuance — the **Issue now** request type. The Add Certificate form also offers **Pre-register**, which registers a certificate to be issued later using a challenge secret. See [Register Certificate](./register-certificate.mdx) for that flow.
...
3. Keep the request type **Issue now** — the certificate request is submitted to the authority immediately
