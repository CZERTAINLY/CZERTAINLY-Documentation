...
Every Connector NG also implements the **Attributes v2 API** (`connector.common.v2`, `/v2/attributes`) —
a definition registry plus a stateless `dependsOn` callback surface for resolving dynamic attribute
content. See [NG Connector Attribute Callbacks (Attributes v2 API)](../../../contributors/attributes/ng-callbacks.mdx).
