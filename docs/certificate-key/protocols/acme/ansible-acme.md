...
[Ansible](https://www.ansible.com/) is a suite of tools to enable the automatization of software configuration. It can be used in conjunction with an ACME server running on the ILM platform to automate certificate management, especially in situations where a target entity isn't capable of certificate management operations and direct communication with an ACME server through available ACME client implementations.

The platform supports ACME implementation according to the [RFC 8555](https://datatracker.ietf.org/doc/html/rfc8555). This guide shows, how you can use Ansible `acme` module to manage certificates using ACME protocol and certificate management services controlled by the platform.
...
To use Ansible with the platform, you need to have the following:
- Ansible installed
  ...
  tasks:
    - name: Set vars
      ansible.builtin.set_fact:
        ...
        bind: "{{ lookup('file', 'vars/ilm.private') | from_yaml }}"
...
The following is an example of challenge data generated by the platform for submitted CSR containing with two domain name identifiers `demo.ilm.test` and `www.ilm.test`:

```json
  "challenge_data": {
     "demo.ilm.test": {
        "dns-01": {
           "record": "_acme-challenge.demo.ilm.test",
     ...
     "www.ilm.test": {
        "dns-01": {
           "record": "_acme-challenge.www.ilm.test",
           "resource": "_acme-challenge",
...
The file `vars/ilm.private` gets loaded every time the playbook is executed. The following Ansible task is responsible for publishing the challenge to desired DNS resolver:
...
Once the challenges are successfully published, Ansible can request the platform to validate them:
