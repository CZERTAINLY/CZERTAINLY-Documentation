...
- **`extension`** — maps the value to an X.509 extension. Carries `extensionOid` (dotted-decimal; must be registered in the [OID registry](../../certificate-key/settings/oid.md)) and `criticalOverridable` — whether the requester may override the extension's default criticality. `criticalOverridable` is reserved — stored but not evaluated yet; criticality always follows the registry's default.
