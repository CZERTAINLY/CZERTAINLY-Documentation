...
If you are using the [External CMS Signer](./pades-signer/external-cms.mdx), you can get the validation data for the signature and embed it into the resulting PDF document without having to implement the validation data collection yourself, achieving the same result as with the PAdES Signer on Baseline signatures levels B-LT and B-LTA.
:::
...
When using batch signatures, the `signatureData` has a form of JSON object representing the batch signature response as defined in [Batch Signing](./batch-signing.mdx). The `validationData` is common to all signatures as the signatures were created by the same key.
